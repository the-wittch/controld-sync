...
def main() -> int:
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Override config and never perform writes"
    )
    parser.add_argument(
        "--check-drift",
        action="store_true",
        help="Fail when a remote folder differs from its source",
    )
    parser.add_argument(
        "--validate", action="store_true", help="Validate remote folders without changing them"
    )
    parser.add_argument(
        "--check-updates",
        action="store_true",
        help="Check source freshness without advancing cache",
    )
    parser.add_argument(
        "--no-cache", action="store_true", help="Disable reading and writing the source cache"
    )
    parser.add_argument(
        "--list-profiles",
        action="store_true",
    ...
    try:
        if args.generate_hagezi_config:
            ...
            print(
                f"Generated {args.generate_hagezi_config} with {count} HaGeZi folders "
        ...
        cache_path = (
            config_dir / str(settings.get("cache_file", ".controld-sync-cache.json"))
        ).resolve()
        ...
        for name, source in folder_sources.items():
            try:
                folders[str(name)] = load_folder_source(
                    str(source)
                    if str(source).startswith(("http://", "https://"))
                    else str((config_dir / str(source)).resolve()),
                    str(name),
                )
            except (SyncError, OSError, ValueError, TypeError) as exc:
                failures.append({"profile": "(source)", "folder": str(name), "error": str(exc)})
                print(f"error: source {name!r}: {exc}", file=sys.stderr)
        ...
        atomic, fail_on_drift = (
            bool(settings.get("atomic_replace", True)),
            args.check_drift or bool(settings.get("fail_on_drift", False)),
        )
        summaries = []
        for profile_name, profile_id in profile_ids.items():
                ...
                failures.append(
                    {
                        ...
                        "folder": "(mapping)",
                        "error": f"[profile_folders] missing mapping for {profile_name!r}",
                    }
                )
                continue
            for folder_name in selected:
                    ...
                    failures.append(
                        {
                            ...
                            "error": f"unknown folder {folder_name!r}",
                        }
                    )
                    print(
                        f"error: [{profile_name}] unknown folder {folder_name!r}", file=sys.stderr
                    )
                    continue
                ...
                try:
                    ...
                    failures.append(
                        {"profile": profile_name, "folder": str(folder_name), "error": str(exc)}
                    )
                    print(f"error: [{profile_name}] {folder_name!r}: {exc}", file=sys.stderr)
                    continue
                if fail_on_drift and (additions or removals):
                    ...
                    print(
                        f"error: [{profile_name}] drift detected in {folder_name!r}",
                        file=sys.stderr,
                    )
                    continue
                summaries.append(
        ...
        should_save = (
            (apply or bool(settings.get("write_cache", False)))
            and not args.check_updates
